...
sleep_duration = 0.176  # default: 85 BPM
...
WARM_YELLOW = 2  # matrix.LED_RED (1) + matrix.LED_GREEN (2)
...
while True:
    ...
    accel_x = (y / adafruit_lis3dh.STANDARD_GRAVITY)
    accel_y = (x / adafruit_lis3dh.STANDARD_GRAVITY)
    accel_z = (z / adafruit_lis3dh.STANDARD_GRAVITY)
    ...
    for x in range(8):
        for y in range(8):
            if last_matrix_state[x][y] != 0:
                lit.append(x * 8 + y + 1)
    print(f"{lit},{accel_z:.2f},{sleep_duration}")
